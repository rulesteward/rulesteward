...
//! BETWEEN rhel8's 1.3.2 and rhel9's 1.4.5) is an internal detail. The variants
...
//! NOTE (2026-06-07): RHEL 9.8 rebased fapolicyd 1.4.3 -> 1.4.5, so rhel9 and
//! rhel10 currently pin the SAME version (1.4.5) and are behaviorally identical;
//! the distinct variants are retained for the CLI surface and future divergence.
//!
...
//! `filehash`; 9.8 rebased to 1.4.5), so a host on an older minor than the pin may
...
pub enum TargetVersion {
    ...
    Rhel8,
    /// RHEL 9 family: fapolicyd 1.4.5 (9.8 rebased 1.4.3 -> 1.4.5).
    Rhel9,
...
impl TargetVersion {
    ...
    pub fn fapolicyd_version(self) -> &'static str {
        match self {
            ...
            // RHEL 9.8 rebased fapolicyd 1.4.3 -> 1.4.5, so rhel9 and rhel10
            // currently share a version; split these arms if a minor diverges.
            TargetVersion::Rhel9 | TargetVersion::Rhel10 => "1.4.5",
        }
    }
}
...
mod tests {
    ...
    fn fapolicyd_version_maps_each_rhel_release() {
        // Grounded in `rpm -q fapolicyd` on the pinned docker images
        // (re-verified 2026-06-07): fapolicyd-1.3.2-1.el8 / 1.4.5-1.1.el9_8 /
        // 1.4.5-1.2.el10_2. RHEL 9.8 rebased 1.4.3 -> 1.4.5, so rhel9 and rhel10
        // now both ship 1.4.5 (behaviorally identical).
        assert_eq!(TargetVersion::Rhel8.fapolicyd_version(), "1.3.2");
        assert_eq!(TargetVersion::Rhel9.fapolicyd_version(), "1.4.5");
