...
///
/// ADDENDUM (#475): `classify(MsgType)` still stays `Opaque` BY DESIGN (see
/// `classify.rs`), so msgtype never enters interval/relational reasoning here
/// or in [`super::compare::disjoint`]'s interval fallback. The one place this
/// changed is `disjoint()`'s Eq/Eq (and Eq/Ne) exact-match path, which now
/// folds msgtype too when both compared values resolve via
/// [`msgtype_resolved_number`] -- see `compare.rs`'s
/// `canonical_decides_value_identity` NOTE for the identity-path detail.
#[must_use]
pub fn canonical_value(ft: FieldType, raw: &str, opts: LintOptions) -> Cow<'_, str> {
    ...
    // (#475: the disjointness prover's Eq/Eq identity path also folds msgtype
    // now, via the same msgtype_resolved_number helper called below -- see the
    // doc addendum above and compare.rs.)
    if ft == FieldType::MsgType {
        let t = raw.trim();
        return match msgtype_resolved_number(t, opts) {
            Some(n) => Cow::Owned(n.to_string()),
            // An unknown name, an AppArmor name with the flag off, or an
            // otherwise unparseable value keeps its trimmed spelling (opaque).
            None => Cow::Borrowed(t),
        };
    }
