//! Parse `stig-refs.toml`: the CDN base URL + the per-product pinned DISA STIG
//! zip. Per-product pins let each RHEL STIG be bumped independently.
//!
//! Generic TOML-loading infrastructure, byte-for-byte the same shape as
//! `tools/sshd-stig-update/src/config.rs` / `tools/auditd-stig-update/src/config.rs`
//! -- no sudo-W04-specific decision-making lives here, so (unlike `xccdf.rs` /
//! `derive.rs`) this module is implemented, not stubbed.
