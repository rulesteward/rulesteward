...
//! consume it.
//!
//! Byte-identical logic (and tests, below) to `tools/sshd-stig-update/src/pin.rs`
//! (the revision scheme is generic, not sshd-specific - both tools draw their
//! pins from the same three DISA STIG documents, per this crate's
//! `stig-refs.toml` comment); kept as a separate copy so this tool has no
//! dependency on the sshd crate/tool, mirroring `config.rs`/`source.rs`'s
//! existing duplication. The "cross-tool drift guard" test near the end of
//! this file's `mod tests` makes that byte-identical claim mechanical.
//!
