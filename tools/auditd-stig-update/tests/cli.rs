...
fn check_empty_selection_file_drifts_against_the_populated_table_exits_1() {
    // A file with no audit-rule Groups derives an empty set, which
    // necessarily drifts against the populated shipped table - this keeps
    // coverage of the zero-selection -> drift -> exit-1 path.
    //
    // The shipped row count reflects DISA RHEL 9 STIG V2R7 -> V2R9, which
    // rewrote 9 identity/login rules into dual-arch syscall form (net +9) and
    // added V-279936 cron_exec (net +2).
...
fn check_real_rhel9_fixture_is_in_sync_with_the_populated_table() {
    // The shipped RHEL9_REQUIRED table is populated from this same fixture's
    // derived output, so the real, non-empty rhel9 fixture matches it
    // exactly: 0 drift - mirrors `xccdf.rs`'s
    // `rhel9_fixture_reproduces_code_table_exactly` through the CLI's `check`
    // subcommand.
...
fn check_file_drift_names_the_removed_v_number() {
    ...
    // the derived set (missing V-258217) drifts against the shipped table -
    // this test additionally proves the diff messages are per-row content, not
    // just a length check.
...
// --- exit-code contract: 2 on error (no xccdf::parse involved) ------------
...
// call happens when `--fixture` is given).
...
// staleness-detection issue-opening path. Rewording
