...
//! logic to `tools/{sshd,auditd}-stig-update`'s `src/source.rs` (generic
//! zip-fetch, not selinux-specific); kept as a separate copy so this tool has no
...
pub fn fetch_xccdf(url: &str) -> Result<String, String> {
    let stem: String = url
        ...
        .collect();
    let work =
        std::env::temp_dir().join(format!("selinux-stig-update-{}-{stem}", std::process::id()));
    // Fresh working dir.
