...
//! STIG control-family table against the official DISA XCCDF.
//!
//! Subcommands:
//!   selinux-stig-update check [--product P]
//!                                # drift gate: derive at the pinned DISA zips and
//!                                # diff vs the shipped tables (exit 1 on drift)
//!   selinux-stig-update derive [--product P] [--file XCCDF]
...
use selinux_stig_update::config::{Config, Product};
use selinux_stig_update::derive::{DerivedControl, code_table, diff_controls};
use selinux_stig_update::{source, xccdf};
...
fn main() -> ExitCode {
    ...
    match run(&args) {
        Ok(code) => code,
        Err(e) => {
            eprintln!("selinux-stig-update: {e}");
            ExitCode::from(2)
        }
    }
}
...
fn print_help() {
    eprintln!(
        "selinux-stig-update - derive + drift-check the selinux se-W01/se-W02 \
         STIG control-family table\n\
         \n\
         USAGE:\n  \
           selinux-stig-update check [--product P] [--file X]   drift gate (exit 1 on drift)\n  \
           selinux-stig-update derive [--product P] [--file X]  print derived table + diff\n\
         \n\
         FLAGS:\n  \
    ...
    if drift {
        println!(
            "\nThe DISA XCCDF changed since the shipped table. Run `derive`, review, and \
             update crates/rulesteward-selinux/src/stig.rs (the control_refs match arms), \
             then re-run `check`."
        );
    }
    Ok(if drift {
...
/// Print paste-ready Rust for a human to reconcile `stig.rs`'s `control_refs`
/// match arms against: one `(family, target) => &[(stig_id, v_number)]` row
/// comment per derived control.
fn print_paste_ready(derived: &[DerivedControl]) {
    println!("# paste-ready control_refs rows:");
    for r in derived {
        println!(
            "#   ({:?}, target) => &[({:?}, {:?})],",
