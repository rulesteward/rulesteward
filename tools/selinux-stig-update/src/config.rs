...
//! Per-product pins let each RHEL STIG be bumped independently. Byte-identical
//! logic to `tools/{sshd,auditd}-stig-update`'s `src/config.rs` (the file format
//! is generic, not selinux-specific); kept as a separate copy so this tool has no
//! dependency on the sshd/auditd crates/tools.
